...
-- "{\\1c&H"..color.."}"
local tethysStyle = {
	button = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)\\fn(%s)}"):format(
		tethys.buttonColor,
		tethys.buttonH,
		tethys.osdSymbolFont
	),
	buttonHovered = genColorStyle(tethys.buttonHoveredColor),
	buttonHoveredRect = ("{\\rDefault\\blur0\\bord0\\1c&H%s\\1a&H%X&}"):format(
		tethys.buttonHoveredRectColor,
		tethys.buttonHoveredRectAlpha
	),
	smallButton = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)\\fn(%s)}"):format(
		tethys.buttonColor,
		tethys.smallButtonSize,
		tethys.osdSymbolFont
	),
	trackButton = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)\\fn(%s)}"):format(
		tethys.buttonColor,
		tethys.trackButtonSize,
		tethys.osdSymbolFont
	),
	trackText = ("{\\fscx%s\\fscy%s\\fn(%s)}"):format(
		tethys.trackTextScale,
		tethys.trackTextScale,
		mp.get_property("options/osd-font")
	),
	windowBar = ("{\\1c&H%s}"):format(tethys.windowBarColor),
	windowButton = ("{\\blur0\\bord(%d)\\1c&H%s\\3c&H000000\\fs(%d)\\fn(%s)}"):format(
		tethys.windowTitleOutline,
		tethys.windowButtonColor,
		tethys.windowButtonSize,
		tethys.osdSymbolFont
	),
	closeButtonHovered = genColorStyle(tethys.closeButtonHoveredColor),
	windowTitle = ("{\\blur0\\bord(%d)\\1c&H%s\\3c&H000000\\fs(%d)}"):format(
		tethys.windowTitleOutline,
		tethys.textColor,
		tethys.windowTitleSize
	),
	buttonTooltip = ("{\\blur0\\bord(1)\\1c&H%s\\3c&H000000\\fs(%d)}"):format(
		tethys.tooltipColor,
	...
	cacheText = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(
		tethys.textColor,
		tethys.cacheTextSize,
		tethys.osdSymbolFont
	),
	seekbar = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(tethys.seekbarFgColor, tethys.seekbarHeight),
	seekbarTimestamp = ("{\\blur0\\bord(%d)\\1c&H%s\\3c&H000000\\fs(%d)}"):format(
		tethys.seekbarTimestampOutline,
		tethys.textColor,
		tethys.seekbarTimestampSize
	),
...
local tethysIcon_play =
	"{\\p1}m 0 0   m 44 44   m 37.236218 17.599999   b 41.584084 20.610064 41.584084 21.923269 37.236218 24.933333   b 22.516553 35.123867 9.369549 44 6.436216 44   b 3.502883 44 3.502883 39.6 3.502883 21.266665   b 3.502883 4.4 3.502883 0 6.436216 0   b 9.369549 0 22.516553 7.409462 37.236218 17.599999{\\p0}"
local tethysIcon_pause =
	"{\\p1}m 0 0   m 44 44   m 17.5 40.2064   b 17.5 45.263808 4.5 45.263808 4.5 40.2107   l 4.5 3.793057   b 4.5 -1.264352 17.5 -1.264352 17.5 3.793057   m 39.5 40.2064   b 39.5 45.263808 26.5 45.263808 26.5 40.2107   l 26.5 3.793057   b 26.5 -1.264352 39.5 -1.264352 39.5 3.793057{\\p0}"
local mpvOsdIcon_close =
	"{\\p1}m 0 0   m 44 44   m 34 34   l 30.571428 34   l 22 25.535715   l 13.535714 34   l 10 34   l 10 30.571428   l 18.464286 22   l 10 13.535714   l 10 10   l 13.535714 10   l 22 18.464286   l 30.571428 10   l 34 10   l 34 13.535714   l 25.535715 22   l 34 30.464285{\\p0}"
local mpvOsdIcon_maximize =
	"{\\p1}m 0 0   m 44 44   m 34 33   l 10 33   l 10 11   l 34 11   m 32 31   l 32 15   l 12 15   l 12 31{\\p0}"
local mpvOsdIcon_minimize = "{\\p1}m 0 0   m 44 44   m 34 25   l 10 25   l 10 19   l 34 19{\\p0}"
local mpvOsdIcon_restore =
	"{\\p1}m 0 0   m 44 44   m 34 25   l 27.999999 25   l 27.999999 33   l 10 33   l 10 18.999999   l 16 18.999999   l 16 11   l 34 11   m 32 23.000001   l 32 15   l 18 15   l 18 18.999999   l 27.999999 18.999999   l 27.999999 23.000001   m 26 31   l 26 23.000001   l 12 23.000001   l 12 31{\\p0}"
-- 32x28
...
function calcTrackButtonWidth(trackArr)
	...
	local trackButtonWidth = math.ceil(
		trackIconWidth + trackDigitWidth * (spaceDigitRatio + numTrackDigits + slashDigitRatio + numTrackDigits)
	)
...
function showThumbnail(thumbState, globalX, globalY)
	...
	mp.command_native({
		"overlay-add",
		thumbState.overlayId,
		globalX,
		globalY,
		thumbState.thumbPath,
		0, -- byte offset
		"bgra", -- image format
		thumbState.globalWidth,
		thumbState.globalHeight,
...
function render_elements(master_ass)
	...
	for n = 1, #elements do
		...
		if element.type == "slider" then
			...
			if pos then
				...
				for _, range in pairs(seekRanges or {}) do
					...
					--     nil
					-- )
					elem_ass:round_rect_ccw(pstart, cachebarY1, pend, cachebarY2, 0, nil)
				end
				elem_ass:draw_stop()
				reset_ass(elem_ass, element)
				...
				-- Note: round_rect_cw(x0, y0, x1, y1, r1, r2)
				elem_ass:round_rect_cw(xp - r, foH - r, xp + r, foH + r, r, nil)
				elem_ass:draw_stop()
				reset_ass(elem_ass, element)
			end
		...
		elseif element.type == "button" then
			...
			local isButton = element.eventresponder
				and (
			...
			if isButton and buttonHovered and element.enabled then
				...
				if tethys.showButtonHoveredRect then
					...
					bgrect_ass:round_rect_cw(0, 0, elem_geo.w, elem_geo.h, 0, 0)
...
function osc_init()
	...
	ne.content = function()
		...
		return ("\238\132\134" .. tethysStyle.trackText .. " " .. aid .. "/" .. #tracks_osc.audio)
	end
	ne.eventresponder["mbtn_left_up"] = function()
		set_track("audio", 1)
	end
	...
	ne.content = function()
		...
		return ("\238\132\135" .. tethysStyle.trackText .. " " .. sid .. "/" .. #tracks_osc.sub)
	end
	ne.eventresponder["mbtn_left_up"] = function()
		set_track("sub", 1)
	end
		...
		if state.rightTC_trem then
			...
			if state.tc_ms then
				...
				return (
					...
					.. " / "
					.. minus
					.. mp.get_property_osd("playtime-remaining")
				)
			end
		else
			if state.tc_ms then
	...
	ne.eventresponder["wheel_up_press"] = function()
		mp.commandv("osd-auto", "add", "speed", 0.25)
	end
	ne.eventresponder["wheel_down_press"] = function()
		mp.commandv("osd-auto", "add", "speed", -0.25)
	end
